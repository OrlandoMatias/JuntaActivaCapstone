    ...
    setup(build) {
      for (const [alias, target] of Object.entries(aliases)) {
        ...
        build.onResolve({ filter }, async args => {
          ...
          // fallback to resolved path (esbuild will try its own resolution)
...
async function ensureDistIndex() {
  ...
  // copy fallback.css if present so dev server can serve it
  try {
...
async function processCss() {
  ...
  // if PostCSS left @tailwind or @apply unexpanded for some reason,
  // run the Tailwind CLI as a fallback to guarantee generated utilities.
...
async function start() {
  ...
  const serveOptions = { servedir: distDir, port: 5000 };
  const buildOptions = {
  ...
  // start initial build with watch
  // ensure CSS is processed first
  await processCss();
  // watch CSS file and re-run PostCSS on changes
  fsSync.watch(path.join(projectRoot, 'client', 'src', 'index.css'), async () => {
    try { await processCss(); } catch (e) { console.error('postcss watch error', e); }
  });
  ...
  const server = http.createServer((req, res) => {
    const reqUrl = decodeURIComponent(new URL(req.url, 'http://localhost').pathname);
  ...
  server.listen(5000, '127.0.0.1', () => {
    // Ensure fallback.css exists in dist on server start (idempotent)
    (async () => {
      try {
        await fs.copyFile(path.join(projectRoot, 'client', 'fallback.css'), path.join(distDir, 'fallback.css'));
      } catch (_) {}
    })().finally(() => {
      console.log('esbuild dev server running at http://127.0.0.1:5000');
    });
  });
}
